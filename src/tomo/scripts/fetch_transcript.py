...
Reverse engineered from ~/.hermes version for Tomo use.

Usage:
    python fetch_transcript.py "YOUTUBE_URL" [--language en,tr] [--timestamps] [--text-only]

Options:
  --text-only     Output plain text (or timestamped lines if also --timestamps) instead of JSON
  --timestamps    Include timestamped_text in JSON or use timestamped format in text-only mode
  --language, -l  Comma-separated language codes (e.g. en,tr). Default: auto-detect
